...
class ExUserManager(UserManager):
    ...
    def create_superuser(self, username, email, password, name='', **extra_fields):
        if not name:
            name = username
        user = self._create_exuser(username, email, password, name, 'SU', True, True,
                                   **extra_fields)
        new_key = user.gen_key()
        user.set_key(new_key, key_expiration=timezone.now() + timezone.timedelta(days=self.model.KEY_LIFE))
        return user
